...
export const AdminDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { id: "pages" as AdminView, icon: FileText, label: isAr ? "الصفحات" : "Pages" },
    ...
    { id: "submissions" as AdminView, icon: Inbox, label: isAr ? "طلبات التواصل" : "Contact Submissions" },
    { id: "footer" as AdminView, icon: PanelBottom, label: isAr ? "الفوتر" : "Footer CMS" },
    { id: "settings" as AdminView, icon: Settings, label: isAr ? "الإعدادات" : "Settings" },
  ];
  ...
  const resolveViewFromPath = (path: string): AdminView => {
    if (path === "/admin/pages/edit") return "page-editor";
  ...
  const renderContent = () => {
    switch (currentView) {
      case "dashboard":
        return <DashboardView isAr={isAr} onNavigate={navigateToView} onEditPage={(id) => { setEditingPage(id); navigateToView("page-editor", { pageId: id }); }} />;
      case "pages":
        return <PagesView isAr={isAr} onEdit={(id) => { setEditingPage(id); navigateToView("page-editor", { pageId: id }); }} />;
      case "blog":
        return <BlogView isAr={isAr} onEditPage={(id) => { setEditingPage(id); navigateToView("page-editor", { pageId: id }); }} />;
      case "page-editor":
        return <PageEditorView isAr={isAr} pageId={editingPage} onBack={() => navigateToView("pages")} />;
      case "partners":
          ...
          {sidebarItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm ${
                currentView === item.id || (currentView === "page-editor" && item.id === "pages")
...
const DashboardView = ({ isAr, onNavigate, onEditPage }: { isAr: boolean; onNavigate: (v: AdminView) => void; onEditPage: (id: string) => void }) => {
  const { pages, contactSubmissions } = useSiteData();
    ...
    <div className="space-y-6">
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <Button className="bg-[#104E8B] hover:bg-[#0A2647] text-white justify-start" onClick={() => onNavigate("pages")}>
          ...
          <Button className="bg-[#104E8B] hover:bg-[#0A2647] text-white justify-start" onClick={() => onNavigate("settings")}>
...
// ──────────────────── Pages View ────────────────────
const PagesView = ({ isAr, onEdit }: { isAr: boolean; onEdit: (id: string) => void }) => {
  const { pages } = useSiteData();
...
// ──────────────────── Page Editor View ────────────────────
const PageEditorView = ({ isAr, pageId, onBack }: { isAr: boolean; pageId: string | null; onBack: () => void }) => {
  const { pages, updateSectionField, toggleSectionVisibility, saveSiteData } = useSiteData();
  ...
  const handleFieldChange = (sectionId: string, fieldKey: string, value: string) => {
    updateSectionField(page.id, sectionId, fieldKey, value, activeTab);
  };

  const handleSave = async () => {
    const ok = await saveSiteData();
    toast.success(ok ? (isAr ? "تم حفظ التعديلات بنجاح" : "Changes saved successfully") : (isAr ? "تعذر الحفظ، أعد المحاولة" : "Failed to save, try again"));
  };

  const getGroupLabel = (sectionId: string, groupKey: string) => {
    ...
    const labels = sectionId === "sms-hero"
      ? heroLabels
      : sectionId === "sms-pricing"
        ? pricingLabels
        : genericLabels;
            ...
            <Card key={section.id} className={`border shadow-sm overflow-hidden ${!section.visible ? "opacity-60" : ""} ${isExpanded ? "border-[#104E8B]/30" : "border-gray-100"}`}>
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`p-1.5 rounded-md transition-colors ${section.visible ? "hover:bg-green-50 text-green-600" : "hover:bg-red-50 text-red-400"}`}
                    title={section.visible ? (isAr ? "مرئي" : "Visible") : (isAr ? "مخفي" : "Hidden")}
                  >
              ...
              {isExpanded && (
                <div className="border-t border-gray-100 p-4 space-y-4 bg-gray-50/30">
                  {!isExternallyManagedSection && section.fields.length > 0 ? (
                    shouldUseGroupedFields(section.id) ? (
                      <div className="space-y-4">
                        {getGroupedFields(section.id, section.fields).map(([groupKey, groupFields]) => (
                          <div key={groupKey} className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                            <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                              <h4 className="text-sm text-[#104E8B]">{getGroupLabel(section.id, groupKey)}</h4>
                              <span className="text-xs text-gray-400">
                                {groupFields.length} {isAr ? "حقول" : "fields"}
                              </span>
                            </div>
                    ...
                    <div className="rounded-lg border border-dashed border-[#104E8B]/30 bg-[#104E8B]/5 p-4 text-sm text-[#104E8B]">
                      <p className="font-medium">
                        {isAr ? "هذا القسم مُدار من مكان آخر في لوحة التحكم." : "This section is managed from another area in the dashboard."}
                      </p>
                      <p className="mt-1 text-xs text-[#104E8B]/80">
                        {isAr ? "مثال: شعارات شركاء النجاح يتم تعديلها من تبويب شركاء النجاح." : "Example: Success partners logos are managed from the Partners tab."}
                      </p>
                    </div>
                  )}
                </div>
